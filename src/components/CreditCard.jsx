    ...
    <div className="relative w-full aspect-[1.6/1] max-w-[400px] mx-auto rounded-[1.25rem] p-6 text-white shadow-xl overflow-hidden cursor-pointer hover:scale-[1.01] transition-transform duration-300" 
      ...
      <div className="relative h-full flex flex-col justify-between z-10 w-full overflow-hidden">
        <div className="flex justify-between items-start w-full">
          <Wifi size={28} className="rotate-90 opacity-80 shrink-0" strokeWidth={2.5} />
          <div className="text-right overflow-hidden flex-1 pl-4">
            <p className="text-[clamp(1rem,5cqw,1.25rem)] sm:text-xl tracking-[0.14em] font-mono font-medium drop-shadow-sm opacity-90 truncate w-full">{cardData.number || '**** **** **** ****'}</p>
            <p className="text-xs font-mono opacity-70 mt-1 mr-1 float-right tracking-widest text-[#94a3b8]">{cardData.expiry || 'MM/YY'}</p>
          </div>
        </div>
        
        <div className="w-full">
          <div className="w-12 h-9 rounded-md bg-gradient-to-br from-[#f8fafc] via-[#cbd5e1] to-[#94a3b8] mb-4 flex items-center justify-center opacity-90 shadow-[0_2px_4px_rgba(0,0,0,0.5)] border border-white/30 shrink-0">
            {/* Chip engrave lines */}
            <div className="w-9 h-6 border border-black/20 rounded-sm grid grid-cols-2 gap-[1px] p-[1px]">
          ...
          <div className="flex justify-between items-end w-full overflow-hidden gap-2">
            <div className="overflow-hidden flex-1 pr-2">
              <p className="text-[0.65rem] uppercase tracking-wider opacity-60 mb-0.5 font-medium text-[#cbd5e1]">Card Holder Name</p>
              <p className="font-semibold text-lg drop-shadow-md tracking-wide truncate w-full">{cardData.name || 'CARD HOLDER'}</p>
            </div>
            <div className="font-bold italic text-3xl tracking-tighter mix-blend-overlay opacity-90 font-serif drop-shadow-lg shrink-0">VISA</div>
          </div>
        </div>
      </div>
    </div>
